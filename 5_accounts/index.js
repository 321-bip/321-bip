...
function addAmount(acccountName, amount) {
  ...
  if (!amount) {
    console.log(
      chalk.bgRed.black("Ocorreu em erro, tente novamente mais tarde!")
    );
    return deposit();
  }
...
function removeAmount(accountName, amount) {
  ...
  if (!amount) {
    console.log(
      chalk.bgRed.black("ocorreu um erro, tente novamente mas tarde")
    );
    return withdraw();
  }
  ...
  if (amount == 0 || accountData.balance == 0) {
    inquirer
      .prompt([
        {
          name: "res",
          type: "confirm",
          message: "voce deseja fazer um deposito?",
        },
      ])
      .then((answer) => {
        if (answer.res === true) {
  ...
  if (accountData.balance >= amount && amount > 0) {
